...
export default function EspacosPage() {
    ...
    return (
        <div className="relative p-4">
            ...
            <p>Esta é a página para gestão de usuários. Você pode adicionar, editar ou remover espaços.</p>

           
            <ul className="mt-4 space-y-2">
                {items.map((item, index) => (
                    <Link href={`espacos/${item}`}>
                        <li key={index} className="p-4 border border-white rounded shadow-sm block">
                            {item}
                        </li>
                    </Link>
                ))}
